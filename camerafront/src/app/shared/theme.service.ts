...
export class ThemeService {
  ...
  public onThemeUpdate: EventEmitter<DocsSiteTheme> =
    new EventEmitter<DocsSiteTheme>();

  constructor(private config: AppConfig, private _styleManager: StyleManager) {}

  storeTheme(theme: DocsSiteTheme): void {
    const locStrgData =
      window.localStorage[this.config.getConfiguration().uuid] || "{}";
  ...
  getStoredTheme(): DocsSiteTheme {
    const locStrgData =
      window.localStorage[this.config.getConfiguration().uuid] || "{}";
    return (
      JSON.parse(locStrgData)[ThemeService.STORAGE_KEY] ||
  ...
  installTheme(theme: DocsSiteTheme): void {
    ...
    if (theme.isDark) {
      this._styleManager.setStyle(
        "theme",
        `./assets/themes/${theme.href_dark}`
      );
      this._styleManager.setStyle(
        "theme-highlight",
        `./assets/themes/${theme.highlight_dark}`
      );
    } else {
      this._styleManager.setStyle("theme", `./assets/themes/${theme.href}`);
      this._styleManager.setStyle(
        "theme-highlight",
        `./assets/themes/${theme.highlight}`
      );
    }
